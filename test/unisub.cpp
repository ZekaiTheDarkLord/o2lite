...
int list_properties()
{
    ...
    while ((pubname = o2_service_name(pub0))) {
        if (o2_service_type(pub0) != O2_TAP &&
            streql(pubname, u8"pubunistr0")) {
            return pub0;
        }
        pub0++;
    }
    printf(u8"Could not find pubunistr0 in services\n");
    assert(false);
    return -1;
}
...
int main(int argc, const char *argv[])
{
    ...
    o2_initialize("unistr");
    
    for (int i = 0; i < n_addrs; i++) {
        char path[128];
        sprintf(path, u8"/subunistr%d", i);
    ...
    assert(o2_service_new(u8"copyunistr0") == O2_SUCCESS);
    assert(o2_method_new(u8"/copyunistr0/äta", "sSi",
                         &copy_sSi, NULL, false, true) == O2_SUCCESS);

    server_addresses = O2_MALLOCNT(n_addrs, char *);
    for (int i = 0; i < n_addrs; i++) {
        char path[100];
        sprintf(path, u8"!pubunistr%d/äta", i);
        server_addresses[i] = O2_MALLOCNT(strlen(path), char);
        strcpy(server_addresses[i], path);
    }

    while (o2_status(u8"pubunistr0") < O2_REMOTE) {
        o2_poll();
        o2_sleep(2); // 2ms
    }
    printf(u8"We discovered pubunistr0 sevice.\ntime is %g.\n",
           o2_time_get());
    ...
    const char *value = 
            o2_service_getprop(pub0, u8"attr_unistr");
    assert(streql(value, u8"value_unistr"));
    O2_FREE((char *) value);
    ...
    assert(pub0 == o2_service_search(0, u8"attr_unistr",
                                     u8"nâtiônà£"));
    ...
    o2_send_cmd(u8"!pubunistr0/äta", 0, "sSi",
                u8"unistr", u8"unistr", 0);
    ...
    value = o2_service_getprop(pub0, u8"attr_unistr");
    assert(value == NULL);
    ...
    search_for_non_tapper(u8"copyunistr0", true);
